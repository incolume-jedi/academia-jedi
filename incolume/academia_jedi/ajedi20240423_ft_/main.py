...
class LogApp:
    def __init__(self, page: ft.Page):
        ...
        self.db_execute('CREATE TABLE IF NOT EXISTS register(cidade, dia1, dia2, obs)')
    ...
    def add_bd(self, c, d1, d2, o1): #funcao que grava os dados coletados no banco
        ...
        if c:
            self.db_execute(query='insert into register values(?, ?, ?, ?)', params=[c, d1, d2, o1])
    ...
    def result(self, e):
        ...
        self.result = self.db_execute(f"select * from register where cidade like '{self.e}%'")
        self.page.update()
    ...
    def new_city(self):
        return ft.Container(
            ...
            content= ft.Column(
                horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                controls=[
                    new_city_ := ft.TextField(ref=self.new_city_, hint_text='Nova cidade'),
                    ...
                    observation := ft.TextField(ref=self.observation, hint_text='Observacao de entrega', min_lines=3, max_lines=5),
                    ft.ElevatedButton(text='Cadastrar', on_click=self.set_value)
                ]
            )
        )
    ...
    def main_page(self):
        ...
        input_bar = ft.Row(
            ...
            controls=[
                ft.TextField(hint_text='Digite a cidade', on_change=self.result),
                ft.FloatingActionButton(icon=ft.icons.NAVIGATE_NEXT_OUTLINED, on_click=self.result)
            ]
        )
        ...
        tabs = ft.Tabs(
            ...
            scrollable=False,
            tabs=[
                ft.Tab(text='Dia da Rota',
                       content=show),
                ft.Tab(text='Cadastrar cidade',
                       content=register),
            ]
        )
