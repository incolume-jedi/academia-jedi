...
from incolume.academia_jedi.ajedi20230117_speed_data_process.fake_data import (
    get_dataset,
    set_dtypes,
)
...
file = Path(gettempdir(), Path(__file__).parts[-2], 'file.txt')
...
def run():
    """Run it."""
    df0 = set_dtypes(get_dataset(1_000_000))
    print(df0)
    print(df0.info())

    # CSV
    it = perf_counter()
    df0.to_csv(f := file.with_suffix('.csv'), index=False)
    it = perf_counter() - it
    ft = perf_counter()
    df0 = pd.read_csv(f)
    ft = perf_counter() - ft
    ...
    # JSON
    it = perf_counter()
    df0.to_json(f := file.with_suffix('.json'), orient='records', indent=2)
    it = perf_counter() - it
    ft = perf_counter()
    df0 = pd.read_json(f)
    ft = perf_counter() - ft
    ...
    # XLSX
    it = perf_counter()
    df0.to_excel(f := file.with_suffix('.xlsx'), index=False)
    it = perf_counter() - it
    ft = perf_counter()
    df0 = pd.read_excel(f)
    ft = perf_counter() - ft
    ...
    # Parquet
    it = perf_counter()
    df0.to_parquet(f := file.with_suffix('.parquet'))
    it = perf_counter() - it
    ft = perf_counter()
    ...
    # Feather
    it = perf_counter()
    df0.to_feather(f := file.with_suffix('.feather'))
    it = perf_counter() - it
    ft = perf_counter()
    df0 = pd.read_feather(f)
    ft = perf_counter() - ft
    logger.debug(
        f'{f.as_posix():15}: {f.stat().st_size:15}; time w:{it}; time r: {ft}',
    )
