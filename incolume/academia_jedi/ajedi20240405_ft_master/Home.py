...
# ruff: noqa: ANN201, ARG002, D101, D102, D107, ERA001, N999
...
# ruff: noqa: ARG002, DTZ011, ANN201, ERA001, D101, D102, D107
...
class Home(UserControl):
    def __init__(self, route):
        ...
        self.bar_chart = BarChart(
            ...
            bottom_axis=ChartAxis(
                labels=[
                    ChartAxisLabel(
                        value=0,
                        label=Container(Text(value='', size=14), padding=10),
                    ),
                    ChartAxisLabel(
                        value=1,
                        label=Container(Text(value='', size=14), padding=10),
        ...
        self.btn_add_sale = OutlinedButton(
            text='Nova Venda',
            icon=icons.ADD_OUTLINED,
            on_click=self.add_sale_clicked,
        )
        ...
        for group_index, group in enumerate(self.bar_chart.bar_groups):
            for rod in group.bar_rods:
                ...
                rod.color = (
                    ...
                    if e.group_index == group_index
                    and e.type == 'PointerHoverEvent'
    ...
    def update_bar_chart(self, data_id, data_value, data_descr):
        ...
        max_y = (
            int(max(data_value) / 10) + 1
        ) * 10  # round to multiples of 10
