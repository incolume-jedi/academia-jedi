...
# ruff: noqa: A002, ANN201, ARG002, D101, D102, D107, E501, ERA001, N999, PLR2004, T201
...
# ruff: noqa: A002, ANN201, ARG002, D101, D102, D107, E501, ERA001, PLR2004, T201
...
class Customers(UserControl):
    ...
    def edit_clicked(self, e):
        ...
        self.route.register_customer.text_new_customer.value = (
            'Editar Cliente:'
        )
    ...
    def create_report_customer(self, e: FilePickerResultEvent):
        ...
        for row in range(len(self.dt_customers.rows)):
            ...
            for col in range(len(self.dt_customers.columns)):
                text.append(
                    self.dt_customers.rows[row].cells[col].content.value,
                ) if col < 4 else None
            data.append(text)
            ...
            self.dt_order_history.rows.append(
                DataRow(
                    cells=[
                        ...
                        DataCell(Text(data[1])),
                        DataCell(
                            Text(f'R${Validator.format_to_currency(data[2])}'),
                        ),
                        DataCell(
                            IconButton(
