...
class TestMunicipios:
    ...
    records: ClassVar[tuple[int, int]] = (5570, 2)

    def test_get_cities_type(self) -> NoReturn:
        """Unittest."""
        assert isinstance(pkg.get_cities(), pkg.pd.DataFrame)

    def test_get_cities_shape(self) -> NoReturn:
        """Unittest."""
        assert pkg.get_cities().shape == self.records

    def test_get_cities_length(self) -> NoReturn:
        """Unittest."""
        assert len(pkg.get_cities()) == self.records[0]
