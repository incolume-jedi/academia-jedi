...
def realfilename(
    ...
    *,
    ...
    ext = ext or ''.join(suffixes)
    sep = '_' if separador else ''
    basename = filebase.stem.split('.')[0]
    filename = filebase.with_name(f'{basename}').with_suffix(f'{ext}')
    result = filename
    while True:
        ...
        result = filename.with_name(f'{basename}{sep}{dig}').with_suffix(
            f'{ext}',
        )
        count += 1
