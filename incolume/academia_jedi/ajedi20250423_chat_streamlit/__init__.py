...
class Chat:
    """Class chat."""
    def navigation(self) -> dict[str, callable]:
        return {
            'login': self.pg_login,
            'chat': self.pg_chat,
        }

    def read_msg(self, user1: str, user2: str) -> list:
        """Read messagens."""
        result = []
        try:
            with self.filename_chat(user1, user2).open('rb') as f:
    ...
    def write_msg(self, user1: str, user2: str, message: dict) -> Path:
        """Write messages."""
        with (filename := self.filename_chat(user1, user2)).open('wb') as f:
            pickle.dump(message, f)
        return filename


    def filename_chat(self, user1: str, user2: str) -> Path:
    ...
    def create_new_user(self, nome: str, senha: str, path: Path = 'users') -> bool:
        """Create new user."""
        filename = Path(__file__).parent / path/ f'{unidecode(nome).replace(' ', '_').casefold()}.pkl'
        ic(filename)
    ...
    def change_pg(self, page_name: str, ):
        """Change page."""
    ...
    def pg_login(self):
        ...
        with tab2.form(key='cadastro'):
            ...
            if st.form_submit_button('Cadastrar'):
                self.create_new_user(nome, senha)
    ...
    def pg_chat(self):
        ...
        mensagens = self.read_msg(user1=userloged, user2=userchat)
    ...
    def main(self):
        ...
        self.navigation[st.session_state['atualpage']]()
