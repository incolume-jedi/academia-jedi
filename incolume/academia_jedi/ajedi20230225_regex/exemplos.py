...
def example8(entrada: str, regex: str = "") -> bool:
    ...
    """
    regex = (
        regex or r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$"
    )
...
def tratativa0():
    ...
    print(f"{locals()=}")
    print(f"{globals()=}")
    print(f">>> {inspect.stack()[0][3]} {inspect.currentframe().f_code.co_name}")
...
def run():
    ...
    functions: typing.Generator[typing.Callable] = (
        value for key, value in globals().items() if key.startswith("tratativa")
    )
